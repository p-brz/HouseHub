...
class AddUserAccessStrategy extends AbstractAccessStrategy {
    ...
    protected function checkSession() {
        ...
        $userId = $sessManager->getSessionVariable(self::USER_ARG);
        return (!is_null($userId) && is_numeric($userId) && $userId >= 0);
    }

    protected function checkParameters(&$parameters) {
        if ($this->checkName($parameters) && $this->checkNickname($parameters) && $this->checkGender($parameters) && $this->checkLogin($parameters) && $this->checkPassword($parameters)) {
